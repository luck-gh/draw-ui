...
# Windows PowerShell
scripts/ask_draw.ps1 -Type wide -Name "screen-name" -Prompt "..."
...
| Flag | 说明 | 默认 |
|------|------|------|
| Bash `--type` / PowerShell `-Type` | `wide`(16:9) / `square`(1:1) / `portrait`(3:4) / `classic`(4:3) | `wide` |
| `--prompt` | 提示词 | 必填 |
| `--frame` | 参考图路径 | — |
| `--ref` | 额外参考图(可重复)| — |
| `--name` | 输出文件名 | auto |
...
中转接口地址:`DRAW_BASE_URL`,默认值为 `https://zenmux.ai/api/vertex-ai`.这个值只是 Google GenAI-compatible 图片接口的 base URL,可以替换成其他兼容中转站
